...
public class RageMeterOverlay implements LayeredDraw.Layer {
    ...
    public void render(GuiGraphics guiGraphics, DeltaTracker deltaTracker) {
        ...
        if (isEnraged) {
            ...
            // Fixed: Only track initial baseline if we weren't enraged before,
            // or if the duration extends PAST our current anchor limit (new ceiling).
            if (this.lastEndTick == -1 || currentDuration > this.initialEnragedDuration) {
            ...
            // Calculate progress and scale safely. Clamping progress to 1.0F ensures extending
            // duration caps the bar visually at its maximum capacity rather than overflowing the sprite frame.
            float progress = Mth.clamp((float) currentDuration / (float) this.initialEnragedDuration, 0.0F, 1.0F);
            targetRatio = Mth.clamp(progress * this.activationRageRatio, 0.0F, 1.0F);
        } else {
            ...
            if (maxRage > 0) {
                // clamp for when the player unequips a curio i.e., attribute < rage
                targetRatio = Mth.clamp((float) (rage / maxRage), 0.0F, 1.0F);
            }
        }
        ...
        if (isEnraged) {
            ...
            int alpha = (int) (overlayAlpha * 0.2F * 255);
            if (alpha > 0) {
