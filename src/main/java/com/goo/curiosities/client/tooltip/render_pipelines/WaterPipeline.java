...
import com.goo.curiosities.client.particle.gui.BubbleGuiParticle;
...
import com.goo.curiosities.util.TooltipUtil;
...
public class WaterPipeline extends TooltipRenderPipeline {
    ...
    private static final ResourceLocation[] CORAL = new ResourceLocation[]{
            Curiosities.loc("tooltip/coral_small"),
            Curiosities.loc("tooltip/coral_big")
    };
    ...
    private void generateMarineObjects() {
        ...
        totalPlants *= (int) CuriositiesClientConfig.CONFIG.WATER_MARINE_OBJECT_AMOUNT_MULTIPLIER.getAsDouble();
        for (int i = 0; i < totalPlants; i++) {
            boolean isTall = random.nextFloat() <= CuriositiesClientConfig.CONFIG.WATER_TALL_CHANCE.getAsDouble();
            boolean isSeagrass = random.nextFloat() <= CuriositiesClientConfig.CONFIG.WATER_SEAGRASS_CHANCE.getAsDouble();
    ...
    protected void renderOverlayPass() {
        ...
        RenderUtil.fillWithUv(CuriositiesRenderTypes.getWaterRenderType(RenderStateShard.NO_DEPTH_TEST),
        ...
        if (Util.getMillis() % CuriositiesClientConfig.CONFIG.WATER_BUBBLE_SPAWN_INTERVAL.getAsInt() == 0
                && CuriositiesClientConfig.CONFIG.WATER_BUBBLE_SIZE_MULTIPLIER.getAsDouble() > 0) {
            ...
            GuiParticle guiParticle = new BubbleGuiParticle(x, y, 400, 0F,
                    (float) (Mth.randomBetween(RANDOM, -0.4F, -1.2F) * CuriositiesClientConfig.CONFIG.WATER_BUBBLE_SPEED_MULTIPLIER.getAsDouble()),
                    (float) (RANDOM.nextIntBetweenInclusive(6, 12) * CuriositiesClientConfig.CONFIG.WATER_BUBBLE_SIZE_MULTIPLIER.getAsDouble()), RANDOM);
