...
public class MatrixPipeline extends TooltipRenderPipeline {
    ...
    protected void modifyColors() {
        this.borderStart = Colors.MATRIX[2];
        this.borderEnd = Colors.MATRIX[1];
    ...
    protected void renderOverlayPass() {
        float textScale = (float) CuriositiesClientConfig.CONFIG.MATRIX_TEXT_SCALE.getAsDouble(); // text size
        int padding = CuriositiesClientConfig.CONFIG.MATRIX_OBJECT_PADDING.getAsInt();


        ShaderInstance shader = CuriositiesRenderTypes.InternalShaders.ENCRYPTED.getInstance();
        if (shader != null)
            shader.safeGetUniform("CellSize").set(10F);

        RenderUtil.fillWithUv(CuriositiesRenderTypes.getEncryptedRenderType(RenderStateShard.LEQUAL_DEPTH_TEST), gui,
                pos.x - 2, pos.y - 2, pos.x + width + 2, pos.y + height + 2, 400, Colors.MATRIX[2]
        );
        ...
        if (currentTime - lastTickTime >= CuriositiesClientConfig.CONFIG.MATRIX_UPDATE_INTERVAL.getAsLong()) {
            lastTickTime = currentTime;
            if (Minecraft.getInstance().level != null) {

                if (raindrops.size() < CuriositiesClientConfig.CONFIG.MATRIX_MAX_DROPLETS.getAsInt() && textScale > 0) {
    ...
    private static class GlitchBox {
        ...
        public void render(GuiGraphics gui) {
            ...
            RenderUtil.fillWithUv(CuriositiesRenderTypes.getEncryptedRenderType(RenderStateShard.LEQUAL_DEPTH_TEST), gui, x, y, x + width, y + height, 400, Colors.MATRIX[2]);
        }
    }
