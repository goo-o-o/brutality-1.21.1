...
public record ItemDescriptionComponent(
    ...
    public static final Codec<ItemDescriptionComponent> CODEC = RecordCodecBuilder.create(instance -> // wrap back into supplier
            instance.group(
            ...
            Codec.FLOAT.optionalFieldOf("cooldown_seconds", null).forGetter(ItemDescriptionComponent::cooldown),
            Codec.STRING.optionalFieldOf("keybind_identifier", null).forGetter(ItemDescriptionComponent::keybindIdentifier)
    ).apply(instance, ItemDescriptionComponent::new));
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, ItemDescriptionComponent> STREAM_CODEC = StreamCodec.composite(
            ...
            ByteBufCodecs.FLOAT.apply(ByteBufCodecs::optional), comp -> Optional.ofNullable(comp.cooldown()),
            ByteBufCodecs.STRING_UTF8.apply(ByteBufCodecs::optional), comp -> Optional.ofNullable(comp.keybindIdentifier()),
            (type, lines, item, cooldown, keybind) ->
                    new ItemDescriptionComponent(type, lines, item, cooldown.orElse(null), keybind.orElse(null))
    );
