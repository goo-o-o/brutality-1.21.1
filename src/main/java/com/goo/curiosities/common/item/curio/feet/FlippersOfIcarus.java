package com.goo.curiosities.common.item.curio.feet;

import com.goo.curiosities.common.CuriositiesServerConfig;
import com.goo.curiosities.common.item.CuriositiesCurioItem;
import net.minecraft.world.item.ItemStack;
import top.theillusivec4.curios.api.SlotContext;

public class FlippersOfIcarus extends CuriositiesCurioItem {
    public FlippersOfIcarus(Properties properties) {
        super(properties);
    }

    @Override
    public void curioTick(SlotContext slotContext, ItemStack stack) {
        if (slotContext.entity().tickCount % 60 == 0 &&
                slotContext.entity().getEyeY() >= CuriositiesServerConfig.CONFIG.FLIPPERS_OF_ICARUS_BURN_HEIGHT.getAsInt() &&
                slotContext.entity().level().canSeeSky(slotContext.entity().blockPosition())) {
            slotContext.entity().igniteForSeconds(4);
        }
    }
}
